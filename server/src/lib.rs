...
impl<M: ConnectionManager> Server<M> {
    ...
    pub async fn update_db_rules<F>(&self, db_name: &DatabaseName<'static>, update: F) -> Result<()>
    where
        F: FnOnce(DatabaseRules) -> DatabaseRules,
    {
        let db = self.config.update_db_rules(db_name, update)?;
        let rules = db.rules.read().clone();
        self.persist_database_rules(db_name, rules).await?;
        Ok(())
    }
